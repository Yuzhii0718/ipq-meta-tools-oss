...
def gen_cdt():
	...
	prc = subprocess.Popen(['python2.7', cdt_path, '-c', configDir, '-o', inDir, '-m', memory], cwd=cdir)
	prc.wait()
...
def gen_part(flash):
	...
	for flash_type in flash.split(","):
		prc = subprocess.Popen(['python2.7', flash_partition_path, '-c', configDir, '-f', flash_type, '-o', inDir], cwd=cdir)
		prc.wait()
...
def gen_bootconfig():
	...
	prc = subprocess.Popen(['python2.7', bootconfig_path, '-c', configDir, '-o', inDir], cwd=cdir)
	prc.wait()
...
def gen_bootldr():
	...
	prc = subprocess.Popen(['python2.7', bootldr_path, '-c', configDir, '-o', inDir, '-m', memory], cwd=cdir)
	prc.wait()
...
def gen_mbn():
	...
	if mbn_v6 != "true":
		prc = subprocess.Popen(['python2.7', bootconfig_path, '-f', inDir + "/openwrt-" + arch + "-u-boot.elf", '-o', inDir + "/openwrt-" + arch + "-u-boot.mbn"], cwd=cdir)
		if os.path.exists(tiny_path):
			prc = subprocess.Popen(['python2.7', bootconfig_path, '-f', inDir + "/openwrt-" + arch + "_tiny" + "-u-boot.elf", '-o', inDir + "/openwrt-" + arch + "_tiny" + "-u-boot.mbn"], cwd=cdir)

	else:
		prc = subprocess.Popen(['python2.7', bootconfig_path, '-f', inDir + "/openwrt-" + arch + "-u-boot.elf", '-o', inDir + "/openwrt-" + arch + "-u-boot.mbn", '-v', "6"], cwd=cdir)

		if os.path.exists(tiny_path):
			 prc = subprocess.Popen(['python2.7', bootconfig_path, '-f', inDir + "/openwrt-" + arch + "_tiny" + "-u-boot.elf", '-o', inDir + "/openwrt-" + arch + "_tiny" + "-u-boot.mbn", '-v', "6"], cwd=cdir)
...
def gen_lk_mbn():
	...
	if mbn_v6 != "true":
		prc = subprocess.Popen(['python2.7', bootconfig_path, '-f', inDir + "/openwrt-" + arch + "-lkboot.elf", '-o', inDir + "/openwrt-" + arch + "-lkboot.mbn"], cwd=cdir)
	else:
		prc = subprocess.Popen(['python2.7', bootconfig_path, '-f', inDir + "/openwrt-" + arch + "-lkboot.elf", '-o', inDir + "/openwrt-" + arch + "-lkboot.mbn", '-v', "6"], cwd=cdir)
	prc.wait()

...
def process_nand_device(pagesize, pages_per_block, total_blocks, entry, nand_type):
	...
	prc = subprocess.Popen(['python2.7', mbn_gen, nand_partition,
...
def process_nor(config_path, flash_type):
	...
	print '\tCreating user partition',
	prc = subprocess.Popen(['python2.7', mbn_gen, nor_partition,
...
def process_norplusnand_device(nor_pagesize, nor_pages_per_block, nor_total_blocks,
	...
	print '\tCreating user partition',
	prc = subprocess.Popen(['python2.7', mbn_gen, norplusnand_partition,
...
def process_emmc(config_path, flash_type):
	...
	prc = subprocess.Popen(['python2.7', ptool, '-x', emmc_partition], cwd=outputdir)
	prc.wait()
	if prc.returncode != 0:
...
def process_norplusemmc(config_path, flash_type):
	...
	print '\tCreating user partition',
	prc = subprocess.Popen(['python2.7', mbn_gen, norplusemmc_partition,
	...
	prc = subprocess.Popen(['python2.7', ptool, '-x', emmc_partition, '-p', '1'], cwd=outputdir)
	prc.wait()
	if prc.returncode != 0:

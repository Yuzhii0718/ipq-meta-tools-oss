...
def CreateGPTPartitionTable(PhysicalPartitionNumber):
    ...
    if SECTOR_SIZE_IN_BYTES == 4096:
        GPTMAIN             = '%snor_gpt_main%d.bin'        % (OutputFolder,k)
        GPTBACKUP           = '%snor_gpt_backup%d.bin'      % (OutputFolder,k)
        GPTBOTH             = '%snor_gpt_both%d.bin'        % (OutputFolder,k)
        RAW_PROGRAM         = '%snor_rawprogram%d.xml'      % (OutputFolder,k)
        RAW_PROGRAM_BLANK   = '%snor_rawprogram%d_BLANK.xml'% (OutputFolder,k)
        PATCHES             = '%snor_patch%i.xml'           % (OutputFolder,k)
    else:
        GPTMAIN             = '%s%sgpt_main%d.bin'        % (OutputFolder, ftype_layout,k)
        GPTBACKUP           = '%s%sgpt_backup%d.bin'      % (OutputFolder, ftype_layout,k)
        GPTBOTH             = '%s%sgpt_both%d.bin'        % (OutputFolder, ftype_layout,k)
        RAW_PROGRAM         = '%s%srawprogram%d.xml'      % (OutputFolder, ftype_layout,k)
        RAW_PROGRAM_BLANK   = '%s%srawprogram%d_BLANK.xml'% (OutputFolder, ftype_layout,k)
        PATCHES             = '%s%spatch%i.xml'           % (OutputFolder, ftype_layout,k)
    ...
    if SECTOR_SIZE_IN_BYTES == 4096:
        FirstLBA    = 48
        LastLBA     = 48               ## Make these equal at first
    else:
        if "boot0" in ftype_layout:
            FirstLBA    = 1024
            LastLBA     = 1024               ## Make these equal at first
        else:
            FirstLBA    = 34
            LastLBA     = 34               ## Make these equal at first

    if HashInstructions['WRITE_PROTECT_GPT_PARTITION_TABLE'] == True:
    ...
    for j in range(len(PhyPartition[k])):
        ...
        if SECTOR_SIZE_IN_BYTES == 4096 and flash_check_once and PhyPartition[k][j]['flash_type']=="nand":
            FirstLBA = 0
            flash_check_once = 0
        print("\n"+"="*78)
        ...
        if HashInstructions['PERFORMANCE_BOUNDARY_IN_KB']>0 and HashInstructions['ALIGN_PARTITIONS_TO_PERFORMANCE_BOUNDARY'] == False:
        ...
        if HashInstructions['ALIGN_PARTITIONS_TO_PERFORMANCE_BOUNDARY'] == True:
        ...
        if SECTOR_SIZE_IN_BYTES == 512 and j%4==0 :
        ...
        if PhyPartition[k][j]['system']=="true":
            Attributes |= 1<<0
        if SECTOR_SIZE_IN_BYTES == 4096 and PhyPartition[k][j]['flash_type']=="nand":
            Attributes |= 1<<3
    ...
    loop = i + 12
    for b in range(4):
        PrimaryGPT[loop] = (((total_blocks - 1)>>(b*8)) & 0xFF) ; loop+=1
    ...
    if SECTOR_SIZE_IN_BYTES == 4096:
        FirstLBA  = 48   ;   i = UpdatePrimaryGPT(FirstLBA,8,i)
    else:
        if "boot0" in ftype_layout:
            FirstLBA  = 1024   ;   i = UpdatePrimaryGPT(FirstLBA,8,i)
        else:
...
def ParseXML(XMLFile):
    ...
    for element in iter:
        ...
        if element.tag=="parser_instructions":
            ...
            if list(element.keys()):
                ...
                for name, value in list(element.items()):
                    ...
                    if name=='name' or name=='filename' :
                        ...
                        if ValidGUIDForm(value) == True:
                            if OutputToCreate == None:
                                OutputToCreate = "gpt"
            ...
            #print "element.tag==is'file' Found a file, NumPhyPartitions=",NumPhyPartitions    ## i.e. just a file tag (usually in legacy)
    ...
    if 'WRITE_PROTECT_BOUNDARY_IN_KB' in HashInstructions:
        if type(HashInstructions['WRITE_PROTECT_BOUNDARY_IN_KB']) == str:
    ...
    if 'SECTOR_SIZE_IN_BYTES' in HashInstructions:
        if type(HashInstructions['SECTOR_SIZE_IN_BYTES']) == str:
    ...
    if 'PERFORMANCE_BOUNDARY_IN_KB' in HashInstructions:
        if type(HashInstructions['PERFORMANCE_BOUNDARY_IN_KB']) == str:
    ...
    if 'GROW_LAST_PARTITION_TO_FILL_DISK' in HashInstructions:
        if type(HashInstructions['GROW_LAST_PARTITION_TO_FILL_DISK']) == str:
    ...
    if 'WRITE_PROTECT_GPT_PARTITION_TABLE' in HashInstructions:
        if type(HashInstructions['WRITE_PROTECT_GPT_PARTITION_TABLE']) == str:
    ...
    if 'ALIGN_PARTITIONS_TO_PERFORMANCE_BOUNDARY' in HashInstructions:
        if type(HashInstructions['ALIGN_PARTITIONS_TO_PERFORMANCE_BOUNDARY']) == str:
    ...
    if 'USE_GPT_PARTITIONING' in HashInstructions:
        if type(HashInstructions['USE_GPT_PARTITIONING']) == str:
    ...
    if 'DISK_SIGNATURE' in HashInstructions:
        if type(HashInstructions['DISK_SIGNATURE']) == str:
    ...
    if 'ALIGN_BOUNDARY_IN_KB' in HashInstructions:
        if type(HashInstructions['ALIGN_BOUNDARY_IN_KB']) == str:
    ...
    if OutputToCreate == "gpt" and HashInstructions['WRITE_PROTECT_GPT_PARTITION_TABLE'] == False:
        hash_w[NumWPregions]['num_sectors']             = 0
        hash_w[NumWPregions]['end_sector']              = 0
        hash_w[NumWPregions]['num_boundaries_covered']  = 0
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "x:f:p:s:t:g:k:v:d:l:", ["xml=", "format=", "partition=", "search_path=", "location=", "sequentialguid=", "use128partitions=", "verbose=", "total_blocks=", "ftype_layout="])

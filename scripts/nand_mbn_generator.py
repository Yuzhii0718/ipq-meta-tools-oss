...
token_stack = []
ignore_tags = ['comment', 'img_name', 'layout', 'image_type']
xml_comment_tag = '!--'
...
def process_data(data, token_stack):
    ...
    if not data_type == "string":
        clean_data = "%02X" % int(ast.literal_eval(clean_data))
    else:
        clean_data = "".join(["%02x" % ord(x) for x in clean_data])
    if not data_length == None:
    ...
    if not data_type == "string" or ((not data_endianness == None) and data_endianness.lower() == "little"):
...
def generate_footer():
    ...
    footer.append("0" * 2 * int(full_file_length - current_file_len))

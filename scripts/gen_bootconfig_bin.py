...
def write_bin_file(filename, data):
    ...
    padding = int((len(data)/2)%4)
    if padding!=0:
        data = data + '00'*(4 - padding)
    for i in range(0, len(data), 2):
        data_item = data[i:i+2]
        if sys.version_info.major >= 3:
            data_item = int(data_item, 16).to_bytes(1, byteorder='little')
        else:
            data_item = chr(int(data_item, 16))
        fp.write(data_item)
    fp.close()
...
#CRC computation for data
    try:
        ...
        print('Error calculating CRC: ' + str(e))
        return 0

def append_crc(filepath):
    crc = 0
    end_magic = 'b0b1b2b3'
    fp = open(filepath, 'rb')
    data = fp.read().encode('hex')
    #print(data[:672])
    for i in range(0,len(data),8):
        if data[i:i+8] == end_magic:
            crc = calculate_crc(data[:i])
            data = data[:i]+crc+data[i+8:]
            write_bin_file(filepath,data)
            fp.close()
            break
    fp.close()
...
# Create user partition
    ...
    for file in files:
        ...
        if prc.returncode != 0:
            ...
            if(flag):
                file_list = [nand_bootconfig, try_bootconfig_0, try_bootconfig_1]
                for file in file_list:
                    print("Generating CRC for "+file+"\n")
                    if(append_crc(file) == 0):
                        print('\nFailed to calculate CRC for '+file+'\n\n')
                        return -1
            print('\n...Bootconfig created\n\n')
...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            opts, args = getopt(sys.argv[1:], "c:o:f:")
        ...
        for option, value in opts:
            if option == "-c":
                ...
                if value and value.lower() == 'crc':
                    flag = 1
                else:
                    print("Warning: Invalid value for -f flag. Expected 'crc'. Using default.")
    else:

...
def main():
    ...
    # input file argument
    parser.add_argument('-f', '--in_files', help='comma separated absolute paths of files to be concatenated'
                                                 '\neg: "-f C:\Temp\myelf1.elf,C:\Temp\myelf2.elf"',
                        type=lambda s: [str(item).strip() for item in s.split(',')],
                        required=True)

    # optional output file argument
    parser.add_argument('-o', '--out_file', help='* give file name OR absolute path OR file name with absolute path *'
